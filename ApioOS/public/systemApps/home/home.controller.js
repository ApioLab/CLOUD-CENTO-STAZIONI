...
angular.module("ApioApplication").controller("ApioHomeController", ["$scope", "socket", "boardsService", "objectService", "currentObject", "$routeParams", "$location", "$timeout", "sharedProperties", "$http", "$window", "$rootScope", "sweet", function ($scope, socket, boardsService, objectService, currentObject, $routeParams, $location, $timeout, sharedProperties, $http, $window, $rootScope, sweet) {
    ...
    $timeout(function () {
            ...
            document.getElementById('ApioIconsContainer').style.opacity = 1;
    },800);
    ...
    $scope.launchApplicationSimple = function (id) {
        ...
        // Correction of containers and icons classes when subapplication is open
        if (document.getElementById("subApplication")) {
            ...
            if (document.getElementById("ApioIconsContainer")) {
                setTimeout(function () {
                    //console.log("DENTRO SETTIMEOUT SUB APPLICATION");
                    if (document.getElementById("app").classList.contains("half_screen_app")) {
                        //console.log("DENTRO HALF CON SUB APPLICATION");
                        document.getElementById("ApioIconsContainer").classList.add("icons_container_half_screen");
                        document.getElementById("ApioApplicationContainer").classList.add("app_container_half_screen");
                        for (var s in l) {
                            if (l.item(s).classList && l.item(s).classList.contains("ApioIconsContainer4")) {
                                l.item(s).classList.remove("ApioIconsContainer4");
                                l.item(s).classList.add("ApioIconsContainer5");
                            }
                        }
                    } else {
                        //console.log("DENTRO ELSE CON SUB APPLICATION");
                ...
                $.get(route + ".html", function (data) {
                    ...
                    for (var i in properties) {
                        for (var j in properties[i].attributes) {
                            ...
                            if (typeof properties[i].attributes[j] === "object" && properties[i].attributes[j].name !== "event" && properties[i].attributes[j].name !== "listener" && properties[i].attributes[j].name !== "propertyname" && properties[i].attributes[j].name !== "writetoserial" && properties[i].attributes[j].name !== "type") {
                    ...
                    // Correction of containers and icons classes to correctly open an App
                    // CASE: ApioApplicationContainer closed
                    if ($("#ApioApplicationContainer").css("display") == "none") {
                        ...
                        // CASE: ApioApplicationContainer already open
                        // correction if half screen was open
                        if (!document.getElementById("app").classList.contains("half_screen_app")) {
                            //console.log("DENTRO CASO APP APERTA");
                            $("#ApioIconsContainer").css("width", "69%");
                        }
                        // correction if half screen was closed
                        var l = document.getElementById("ApioIconsContainer").childNodes;
                        if (document.getElementById("app").classList.contains("half_screen_app")) {
                            console.log("DENTRO HALF CON APP APERTA");
                            document.getElementById("ApioIconsContainer").classList.add("icons_container_half_screen");
                            document.getElementById("ApioApplicationContainer").classList.add("app_container_half_screen");
                            for (var s in l) {
                                if (l.item(s).classList && l.item(s).classList.contains("ApioIconsContainer2")) {
    ...
    $scope.launchApplicationSimple1 = function (id) {
        var head = document.getElementsByTagName("head").item(0);
        ...
        if (Apio.currentApplication !== Number(id) && !document.getElementsByClassName("popover-content").item(0)) {
            ...
            objectService.getById(id).then(function (d) {
                ...
                $.get(route + ".html", function (data) {
                    ...
                    // Correction of containers and icons classes to correctly open an App
                    // CASE: ApioApplicationContainer closed
                    if ($("#ApioApplicationContainer").css("opacity") == "0") {
                        if (window.innerWidth > 769) {
                            if (document.getElementById("ApioIconsContainer")) {
                                var l = document.getElementById("ApioIconsContainer").childNodes;
                                if (document.getElementById("app").classList.contains("half_screen_app")) {
                        ...
                        $timeout(function () {
                            if (document.getElementById("imageId" + id)) {
                                Apio.stopApioLoading();
                                document.getElementById('ApioApplicationContainer').classList.add('openApp');
                            }
                        });
    ...
    if ($routeParams.hasOwnProperty("application")) {
        ...
        setTimeout(function () {
            $scope.launchApplicationSimple1($routeParams.application);
