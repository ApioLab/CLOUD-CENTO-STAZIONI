...
angular.module('ApioApplication').service('lazyLoadApi', function lazyLoadApi($window, $q) {
    ...
    socket.on("apio_server_delete", function (objectId) {
        for (var i = 0; i < $scope.objects.length; i++) {
            if (objectId === $scope.objects[i].objectId) {
                $scope.objects.splice(i--, 1);
            }
        }
    });

    socket.on("apio_server_new", function (objectId) {
        objectService.getById(objectId).then(function (d) {
            ...
            if (d.data.hasOwnProperty("group")) {
                if (d.data.group.hasOwnProperty("name") && d.data.group.hasOwnProperty("path")) {
                    if ($scope.category.length === 0) {
                        $scope.category.push({
                            name: d.data.group.name,
                            path: d.data.group.path,
                            id: d.data.group.id
                        });
                    } else {
                        for (var n in $scope.category) {
                            if ($scope.category[n].name == d.data.group.name) {
                                break;
                    ...
                    if ($scope.category.length === 0) {
                        ...
                        for (var n in $scope.category) {
                            if ($scope.category[n].name == d.data.group) {
                                break;
                            } else if ($scope.category.length - 1 == n) {
                                $scope.category.push({
                                    name: d.data.group,
            ...
            if (d.data.marker === undefined || Object.keys(d.data.marker).length === 0 || d.data.marker.lat === "" || d.data.marker.lng === "") {
                $scope.asMarker[d.data.objectId] = false;
            } else {
                markers.push(d.data.marker);
                $scope.asMarker[d.data.objectId] = true;
            }
        });
    });
