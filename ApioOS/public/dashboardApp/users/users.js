...
angular.module("ApioDashboardApplication").controller("ApioDashboardUsersController", ["$scope", "socket", "userService", "objectService", "$http", "$rootScope", "$state", "sweet", "$window", function ($scope, socket, userService, objectService, $http, $rootScope, $state, sweet, $window) {
    ...
    //da modificare dopo, dovrà essere in un altro file o db
    $scope.subroles = ["callcenter", "direttore", "operatore"];
    
    $scope.saveUser = function() {
	    location.reload();
    }
    
    $scope.setSubrole = function (sr) {
        console.log("Famo apri sta modal");
        $scope.currentUser.subrole = $scope.subrole = sr;
        console.log("$scope.currentUser.role: ", $scope.currentUser.subrole);
        $("#addUser").modal("hide");
        $http.post("/apio/user/setSubrole", {
            email: $scope.currentUser.email,
            subrole: {type: $scope.currentUser.subrole}
            ...
            if (data.error) {
                ...
                sweet.show({
                    title: "Error!",
                    text: "Subrole cannot be changed",
                    type: "error",
                ...
                sweet.show({
                    title: "Done!",
                    text: $scope.currentUser.email + "'s subrole changed!",
                    type: "success",
                    showCancelButton: false,
    ...
    $scope.launchUser = function (user) {
        ...
        $http.post("/apio/user/getUser", {
            ...
            $scope.subrole = data.user.subrole.type;
            $("#userModal").modal();
            if (document.getElementById(user.email)) {
    ...
    $scope.shareBoard = function () {
        if ($scope.newUser) {
            $http.post("/apio/user/getUser", {
                ...
                if (uData.user && Object.keys(uData.user).length) {
                    console.log("User already in the board");
                } else {
                    if ($scope.config.type === "cloud") {
                        socket.emit("ask_user", {
                            apioId: $scope.session.apioId,
                            user: $scope.newUser,
                            role: $scope.newRole || "guest"
                        });
                ...
                console.log("Error while getting user information: ", userError);
            });
        } else {
            sweet.show({
                title: "Before proceed you have to insert an e-mail",
                type: "error",
                closeOnConfirm: true,
                showCancelButton: false,
        ...
        socket.on("get_user", function (data) {
            if (data.type === "error") {
                sweet.show({
                    title: "Encountered an error while adding the user",
                    text: typeof data.data === "object" ? JSON.stringify(data.data) : data.data,
                    type: "error",
                    closeOnConfirm: true,
                    showCancelButton: false,
                    showLoaderOnConfirm: true
                });
            } else if (data.type === "no_user") {
                sweet.show({
                    title: "User not found in the cloud, create him in this board",
                    type: "error",
                    closeOnConfirm: true,
                    showCancelButton: false,
                    showLoaderOnConfirm: true
                ...
                sweet.show({
                    title: "User successfully enabled",
                    type: "success",
                    closeOnConfirm: true,
                    showCancelButton: false,
                    showLoaderOnConfirm: true
