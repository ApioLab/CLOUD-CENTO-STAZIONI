...
ApioDashboardApplication.controller("ApioDashboardGeneralController", ["$scope", "userService", "boardsService", "objectService", "$state", "$timeout", "$http", "sweet", "socket", "$window", "$rootScope", "$location", function ($scope, userService, boardsService, objectService, $state, $timeout, $http, sweet, socket, $window, $rootScope, $location) {
    ...
    $scope.cancelSaveNewBoardName = function () {
        $scope.modifyBoardName = false;
    };

    $scope.enableModifyBoardName = function () {
        $scope.modifyBoardName = true;
    };

    $scope.saveNewBoardName = function () {
        $http.post("/apio/boards/setName", {
            apioId: $scope.session.apioId,
            name: $scope.boardName
        }).success(function () {
            sweet.show({
                title: "Board name successfully changed",
                type: "success",
                closeOnConfirm: true,
            ...
            sweet.show({
                title: "Error while changing Board name",
                text: typeof error === "object" ? JSON.stringify(error) : error,
                type: "error",
                closeOnConfirm: true,
                showCancelButton: false,
                showLoaderOnConfirm: true
