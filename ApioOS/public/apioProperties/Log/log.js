...
}).directive("log", ["currentObject", "$http", "logToShow", "$timeout", function (currentObject, $http, logToShow, $timeout) {
    return {
        restrict: "E",
        replace: true,
        scope: {
                    ...
                    $.get(uri, function (data) {
                        if (scope.anchor === "<") {
                            ...
                            if (!node) {
                                ...
                                $("#ApioApplicationLog").css("height", "99.8%");
                                if (window.innerWidth < 992) {
                                    //alert('');
                                    //$("#ApioApplicationContainer").css("-webkit-transform", "translateX(" + (-Apio.appWidth) + "px) !important");
                                    //$("#ApioApplicationContainer").css("transform", "translateX(" + (-Apio.appWidth) + "px) !important");
                                    $timeout(function () {
                                        //alert();
                                        scope.progressShow = false;
                                        var ApioApplicationContainerStyle = document.getElementById('ApioApplicationContainer').getAttribute('style');
                                        document.getElementById('ApioApplicationContainer').setAttribute('style', ApioApplicationContainerStyle + " -webkit-transform: translateX(" + (-Apio.appWidth) + "px) !important; transform: translateX(" + (-Apio.appWidth) + "px) !important;");
                                    });
                                }
                            }
                        }
