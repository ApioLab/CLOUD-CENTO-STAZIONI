angular.module("ApioApplicationLog", ["apioProperty"]).controller("defaultController", ["$scope", "currentObject", "socket", "$http", "$timeout", "SaveAs", "logToShow", "$location", function ($scope, currentObject, socket, $http, $timeout, SaveAs, logToShow, $location) {
    ...
    $scope.exportToXLSX = function () {
        ...
        if ($scope.mode === "all" || ($scope.mode === "period" && $scope.periodData.from && $scope.periodData.to)) {
            ...
            if ($scope.xslx_ready === false) {
                ...
                if (ua.indexOf("safari") > -1 && ua.indexOf("chrome") === -1) {
                    ...
                    $http.post("/apio/service/log/route/" + encodeURIComponent("/apio/log") + "/data/" + encodeURIComponent(JSON.stringify({
                            from: $scope.fromDate,
                            to: $scope.toDate,
                            objectId: $scope.object.objectId,
                            type: $scope.function
                        ...
                        for (var i in logs) {
                            for (var j in logs[i]) {
                                ...
                                if (index > -1) {
                                    ...
                                    if (isNaN(data[index][$scope.tableProperties.indexOf(i) + 1])) {
                                        data[index][$scope.tableProperties.indexOf(i) + 1] = logs[i][j];
                                    }
                                } else {
                                    var arr = [new Date(Number(j))];
                                    arr[$scope.tableProperties.indexOf(i) + 1] = Number(logs[i][j].replace(",", "."));
                                    if (isNaN(arr[$scope.tableProperties.indexOf(i) + 1])) {
                                        arr[$scope.tableProperties.indexOf(i) + 1] = logs[i][j];
                                    }
                                    data.push(arr);
                                }
                            }
            ...
            if ($scope.mode === "period") {
                if (!$scope.periodData.from) {
                    alert("Manca la data iniziale del periodo");
                } else if (!$scope.periodData.to) {
                    alert("Manca la data finale del periodo");
                } else {
                    alert("Dati mancanti");
                }
            } else {
    ...
    $scope.$watch("mode", function (newValue) {
        if (newValue === "period") {
            $scope.periodPicker = true;
        } else if (newValue === "all") {
            $scope.periodPicker = false;
            $scope.periodData = {
                from: "",
                to: ""
            };
    ...
    $scope.$watch("periodData.from", function (newValue) {
        var date = newValue.split(" ")[0];
        var dateComponents = date.split("/");
        $scope.fromDate = dateComponents[2] + "-" + dateComponents[1] + "-" + dateComponents[0];
    });

    $scope.$watch("periodData.to", function (newValue) {
        var date = newValue.split(" ")[0];
        var dateComponents = date.split("/");
        $scope.toDate = dateComponents[2] + "-" + dateComponents[1] + "-" + dateComponents[0];
    });
