...
apioProperty.directive("selector", ["currentObject", "socket", "$http", function (currentObject, socket, $http) {
    return {
        restrict: "E",
        replace: true,
        scope: {
        ...
        templateUrl: "/apioProperties/Selector/selector.html",
        link: function (scope, elem, attrs) {
            ...
            elem.on(event, function () {
                if (!currentObject.isRecording()) {
                    ...
                    if(Number(scope.model)+scope.step<scope.max){
                    	scope.model = String(Number(scope.model)+scope.step)
                    }else {
            ...
            */
            scope.up = function(){
            	if (!currentObject.isRecording()) {
	            	if((Number(scope.model)+Number(scope.step))<=Number(scope.max)){
	            		//alert("UP")
	                	scope.model = String(Number(scope.model)+Number(scope.step))
	                }else {
	                    scope.model = scope.max
	                }
	                scope.object.properties[scope.propertyname] = scope.model;
            ...
            scope.down = function(){
	            if (!currentObject.isRecording()) {
	            	...
	            	if((Number(scope.model)-Number(scope.step))>=Number(scope.min)){
	            		//alert("DOWN")
	                	scope.model = String(Number(scope.model)-Number(scope.step))
	                }else {
