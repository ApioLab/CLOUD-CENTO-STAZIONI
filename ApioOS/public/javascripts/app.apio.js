...
var touchstart;
var touchend;
document.getElementById('systemAppContainer').addEventListener('touchstart', function(e){
	touchstart = e.touches[0].changedTouches[0].clientY;
	console.log("touchstart",touchstart);
})

document.getElementById('systemAppContainer').addEventListener('touchend', function(e){
	touchend = e.touches[0].changedTouches[0].clientY;
...
var ApioApplication = angular.module("ApioApplication", ["ui.bootstrap", "ngRoute", "hSweetAlert", "angularFileUpload", "fileSaver", "ngFileReader", "ngMaterial", "md.data.table"]);
...
ApioApplication.service('lazyLoadCharts', function lazyLoadCharts($window, $q) {
    ...
    function loadScript() {
        ...
        s.src = "apioProperties/Charts/charts.js";
...
ApioApplication.controller("ApioMainController", ["$scope", "$http", "socket", "sweet", "$window", "$timeout", "$rootScope", "$location", "$mdDialog", "lazyLoadCharts", function ($scope, $http, socket, sweet, $window, $timeout, $rootScope, $location, $mdDialog, lazyLoadCharts) {
    var configuration;
    ...
    lazyLoadCharts.then(function (data) {
        if (data) {
            ...
            sweet.show({
                ...
                text: "The charts property won't work, restore the connection to use it",
                type: "warning",
                ok: "Close"
            });
        }
    ...
    socket.on("uncompleted_detected", function (data) {
        $scope.failedUpdate = data;
        if (!$scope.$$phase) {
            $scope.apply();
        }

        if ($scope.failedUpdate === "recovered") {
            sweet.show({
                title: "System successfully repaired!",
    ...
    socket.on("update_system", function (data) {
        if ($scope.role && $scope.role !== "guest") {
