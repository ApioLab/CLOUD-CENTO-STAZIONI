...
var systemIsRepairing = undefined;
...
module.exports = function (enableCloudSocket) {
        ...
        if (!Apio.hasOwnProperty("io")) {
            ...
            Apio.io.on("connection", function (socket) {
                    ...
                    f.on("finish", function () {
                        fs.stat("/tmp/" + data.name, function (err, stats) {
                            if (err) {
                                ...
                                if (data.minTs != null) {
                                    sql_db.query("DELETE FROM `" + data.table + "` WHERE `timestamp` >= '" + data.minTs + "'", function (s_e) {
                                        if (s_e) {
                                            console.log("Error while deleting data from table " + data.table + ": ", s_e);
                                        } else {
                                            console.log("Data successfully delete from table " + data.table);
                                            final_query();
                                        }
                                    });
                                } else {
                ...
                socket.on("ask_user", function (data) {
                    Apio.Database.db.collection("Users").findOne({email: data.user}, function (error, user) {
                        var sendData = {
                            type: "no_user",
                            data: null
                        };
                        ...
                        if (error) {
                            ...
                            Apio.Database.db.collection("Users").update({email: data.user}, {
                                $push: {
                                    apioId: {
                                        code: data.apioId,
                                        role: data.role
                                ...
                                if (error) {
                                    console.log("Error while adding the user " + data.user + " to the board " + data.apioId + " with the role " + data.role + ": ", error);
                                } else {
                                    console.log("The user " + data.user + " was correctly added to the board " + data.apioId + " with the role " + data.role);
                                }
                            });
                        }

                        for (var u in Apio.connectedSockets) {
                            if (u === "admin" || validator.isEmail(u)) {
                                for (var i in Apio.connectedSockets[u]) {
                                    if (data.apioId === Apio.io.sockets.connected[Apio.connectedSockets[u][i]].client.request.session.apioId) {
                                        Apio.io.sockets.connected[Apio.connectedSockets[u][i]].emit("get_user", sendData);
                                    }
                                }
                            }
                        }

                        var socketIdGateway = Apio.connectedSockets[data.apioId][0];
                        Apio.io.sockets.connected[socketIdGateway].emit("get_user", sendData);
                    });
                });
                ...
                socket.on("apio.upload.app", function (data) {
                    ...
                    Apio.Database.db.collection("Objects").insert(data.file.mongo, function (err) {
                        if (err) {
                            ...
                            //         for (var i in socketIds) {
                            //             if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                            //                 Apio.io.sockets.connected[socketIds[i]].emit("apio_server_new", data.file.mongo.objectId);
                            //             }
                            //         }
                            //     }
                            // }
                ...
                socket.on("apio.git.clone.app", function (data) {
                    Apio.Database.db.collection("Objects").insert(data.mongo, function (err) {
                        if (err) {
                            ...
                            //         for (var i in socketIds) {
                            //             if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                            //                 Apio.io.sockets.connected[socketIds[i]].emit("apio_server_new", data.objectId);
                            //             }
                            //         }
                            //     }
                            // }
                ...
                socket.on("send_to_service", function (data) {
                    if (Apio.servicesSocket[data.service]) {
                ...
                //                                                             for (var i in socketIds) {
                //                                                                 if (apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                //                                                                     Apio.io.sockets.connected[socketIds[i]].emit("update_system", {
                //                                                                         type: "done"
                //                                                                     });
                //                                                                 }
                //                                                             }
                                        ...
                                        if (error) {
                                            ...
                                            fs.writeFile(path, body, function (e_w) {
                                                if (e_w) {
                                                    ...
                                                    exec("bash " + path, function (error_exec) {
                                                        if (error_exec) {
                                                            console.log("Error while executing file " + path + ": ", error_exec);
                                                        } else {
                                                            fs.unlink(path, function (err_u) {
                                                                if (err_u) {
                                                                    ...
                                                                    var checkNpm = function (cb, final) {
                                                                        fs.readFile("./npm_error.log", "utf8", function (npm_e, npm_content) {
                                                                            if (npm_e && cb) {
                                                                                cb(final);
                                                                            } else if (npm_content.indexOf("ERR") > -1) {
                                                                                exec("bash ./relaunch_npm.sh", function () {
                                                                                    checkNpm(cb, final);
                                                                                    fs.readFile("./bower_error.log", "utf8", function (bower_e, bower_content) {
                                                                                        if (bower_e || bower_content.indexOf("ERR") === -1) {
                                                                                            final();
                                                                                        }
                                                                                    });
                                                                                });
                                                                    ...
                                                                    var checkBower = function (final) {
                                                                        fs.readFile("./bower_error.log", "utf8", function (bower_e, bower_content) {
                                                                            if (!bower_e && bower_content.indexOf("ERR") > -1) {
                                                                                exec("bash ./relaunch_bower.sh", function () {
                                                                                    checkBower(final);
                                                                                    final();
                                                                                });
                                                                            } else {
                                                                    ...
                                                                    var final = function () {
                                                                        for (var x in Apio.connectedSockets) {
                                                                            if (x === "admin" || validator.isEmail(x)) {
                                                                                ...
                                                                                for (var i in socketIds) {
                                                                                    if (apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                                                                                        Apio.io.sockets.connected[socketIds[i]].emit("update_system", {type: "done"});
                                                                                    }
                                                                                }
                                                                            }
                                                                        }
                ...
                socket.on("apio_server_new", function (data) {
                    ...
                    //         for (var i in socketIds) {
                    //             if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                    //                 Apio.io.sockets.connected[socketIds[i]].emit("apio_server_new", data);
                    //             }
                    //         }
                    //     }
                    // }
                ...
                socket.on("apio_server_delete", function (data) {
                    ...
                    /*data.apioId = Apio.System.getApioIdentifier();
                     ...
                     for (var i in socketIds) {
                     if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                     Apio.io.sockets.connected[socketIds[i]].emit("apio_server_delete", data);
                     }
                     }
                     }
                     }*/
                ...
                socket.on("apio.create.new.app", function (data) {
                    ...
                    Apio.Database.registerObject(mongo, function (error) {
                        if (error) {
                            ...
                            //         for (var i in socketIds) {
                            //             if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                            //                 Apio.io.sockets.connected[socketIds[i]].emit("apio_server_new", data.obj.objectId);
                            //             }
                            //         }
                            //     }
                            // }
                ...
                socket.on("apio.server.user.updateUser", function (usr) {
                    Apio.Users.shareApp(usr, function (err, data) {
                        if (err) {
                            ...
                            var socketIds = Apio.connectedSockets[usr.email];
                            for (var i in socketIds) {
                                if (data.apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                                        ...
                                        if (!err) {
                                            ...
                                            data.apio.objects.forEach(function (object) {
                                                ...
                                                if (Object.keys(object.properties).length) {
                                                    var table_name = object.objectId + "_" + data.apio.system.apioId;
                                                    sql_db.query("SHOW TABLES LIKE '" + table_name + "'", function (error_st, result_st) {
                                                        if (error_st) {
                                                            console.log("SHOW TABLES error: ", error_st);
                                                        } else if (result_st) {
                                                            if (result_st.length) {
                                                                //TABLE EXISTS
                                                                var final = function (table, database, field, type) {
                                                                ...
                                                                sql_db.query("SHOW COLUMNS FROM `" + table_name + "`", function (error, cols) {
                                                                    if (error) {
                                                                        console.log("Error while showing cols: ", error);
                                                                    } else if (cols) {
                                                                        var columns = [];
                                                                        for (var x in cols) {
                                                                            if (cols[x].Field !== "id" && cols[x].Field !== "timestamp") {
                                                                ...
                                                                sql_db.query("DROP TABLE IF EXISTS `" + object.objectId + "_" + data.apio.system.apioId + "`", function (error_drop) {
                                                                    if (error_drop) {
                                                                        console.log("Error while creating table: ", error);
                                                                    } else {
                                                                        sql_db.query("CREATE TABLE `" + object.objectId + "_" + data.apio.system.apioId + "` (" + condition_string + ")", function (error, result) {
                                                                            if (error) {
                                                                                console.log("Error while creating table: ", error);
                                                                            } else if (result) {
                                                            ...
                                                            console.log("No result for SHOW TABLES");
                                                        }
                                                    });
                                                }
                                            });
    ...
    Apio.Database.registerObject = function (objectData, callback) {
        Apio.Database.db.collection("Objects").insert(objectData, function (error) {
            ...
            //             if (["apiobutton", "apiolink", "asyncdisplay", "autocomplete", "battery", "charts", "collapse", "dynamicview", "graph", "list", "log", "moreinfo", "note", "paint", "property", "ranking", "text", "textbox", "uploadimage"].indexOf(objectData.properties[p].type.toLowerCase()) > -1) {
            //                 condition_array.push(p + " TEXT");
            ...
            for (var p in objectData.properties) {
                ...
                if (["number", "trigger", "unclickabletrigger"].indexOf(objectData.properties[p].type.toLowerCase()) > -1) {
                    condition_array.push("`" + p + "` INT");
                } else if (["sensor", "slider", "unlimitedsensor"].indexOf(objectData.properties[p].type.toLowerCase()) > -1) {
                    condition_array.push("`" + p + "` DOUBLE");
                } else {
            ...
            sql_db.query("DROP TABLE IF EXISTS `" + table + "`", function (error_drop) {
                if (error_drop) {
                    console.log("Error while creating table: ", error);
                } else {
                    sql_db.query("CREATE TABLE `" + table + "` (" + condition_string + ")", function (error, result) {
                ...
                if (err) {
                    ...
                    for (var p in protocols) {
                        ...
                        for (var a in addresses) {
                            var properties = Object.keys(addressBindToProperty[protocols[p]][addresses[a]]);
                            for (var prop in properties) {
                                if (addressBindToProperty[protocols[p]][addresses[a]][properties[prop]] && typeof addressBindToProperty[protocols[p]][addresses[a]][properties[prop]] === "object") {
                                    if (addressBindToProperty[protocols[p]][addresses[a]][properties[prop]].hasOwnProperty(id)) {
                                        delete addressBindToProperty[protocols[p]][addresses[a]][properties[prop]][id];
                                    }
    ...
    Apio.Users.delete = function (usr, callback) {
        ...
        if (Apio.Configuration.type === "cloud") {
            Apio.Database.db.collection("Objects").update({apioId: usr.apioId}, {$pull: {user: {email: email}}}, {multi: true}, function (error_obj) {
                if (error_obj) {
                    console.log("Error while updating objects with apioId " + usr.apioId + ": ", error_obj);
                    callback("500", null);
                } else {
            ...
            Apio.Database.db.collection("Objects").update({}, {$pull: {user: {email: email}}}, {multi: true}, function (error_obj) {
                if (error_obj) {
                    console.log("Error while updating objects: ", error_obj);
                    callback("500", null);
                } else {
                    Apio.Database.db.collection("Users").update({}, {$pull: {user: {email: email}}}, {multi: true}, function (error) {
                        if (error) {
    ...
    Apio.Users.setSubrole = function (usr, callback) {
        var email = usr.email;
        var subrole = usr.subrole;
        var apioId = undefined;
        ...
        if (Apio.Configuration.type === "cloud") {
            ...
            Apio.Database.db.collection("Users").update({email: email}, {$set: {subrole: subrole}}, function (err) {
                if (err) {
                    callback(err);
                } else {
	                //METTERE SOCKET DI AGGIORNAMENTO
                    callback(null);
                }
            });
        }
    ...
    Apio.Object.install = function (data, objectId, callback) {
                    ...
                    if (e_mkdir) {
                                ...
                                Apio.Database.db.collection("Objects").insert(collection, function (err_db) {
                                    if (err_db) {
                                        ...
                                        if (Apio.Configuration.type === "gateway" && Apio.Configuration.remote.enabled) {
                                            console.log("entro if invio socket");
                                            Apio.Remote.socket.emit("send_to_service", {
                                                data: data,
                        ...
                        fs.stat(baseDir + "/utils", function (err, stats) {
                            if (err) {
                                ...
                                fs.copy(baseDir + "/utils", "public/applications/" + objectId + "/utils", function (err) {
                                    if (err) {
                                        console.log("Error while copying from " + source + " to " + destination + ": ", err);
                                    } else {
                                        console.log("done!");
                ...
                if (typeof addressBindToProperty[protocols[p]] === "object") {
                    ...
                    for (var a = 0; a < addresses.length; a++) {
                        ...
                        for (var bp = 0; bp < bindedProperties.length; bp++) {
                            if (addressBindToProperty[protocols[p]][addresses[a]][bindedProperties[bp]] && typeof addressBindToProperty[protocols[p]][addresses[a]][bindedProperties[bp]] === "object" && addressBindToProperty[protocols[p]][addresses[a]][bindedProperties[bp]].hasOwnProperty(searchQuery.objectId)) {
        ...
        Apio.Database.db.collection("Objects").update(searchQuery, {$set: {properties: newProperties}}, function (o_e) {
            if (o_e) {
                ...
                props.forEach(function (p) {
                    numberOfProperties++;
                    var colType = "";
                    // if (["apiobutton", "apiolink", "asyncdisplay", "autocomplete", "battery", "charts", "collapse", "dynamicview", "graph", "list", "log", "moreinfo", "note", "paint", "property", "ranking", "text", "textbox", "uploadimage"].indexOf(newProperties[p].type) > -1) {
                    //     colType = "TEXT";
                    ...
                    if (["number", "trigger", "unclickabletrigger"].indexOf(newProperties[p].type) > -1) {
                        colType = "INT";
                    } else if (["sensor", "slider", "unlimitedsensor"].indexOf(newProperties[p].type) > -1) {
                        colType = "DOUBLE";
                    } else {
    ...
    Apio.System.setRepair = function (val) {
        var self = this;
        var apioId = Apio.System.getApioIdentifier();
        systemIsRepairing = val;

        this.interval = this.interval || setInterval(function () {
                if (systemIsRepairing) {
                    for (var x in Apio.connectedSockets) {
                        if (x === "admin" || validator.isEmail(x)) {
                            var socketIds = Apio.connectedSockets[x];
                            for (var i in socketIds) {
                                if (apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                                    Apio.io.sockets.connected[socketIds[i]].emit("uncompleted_detected", "trying");
                                }
                            }
                        }
                    }
                } else if (systemIsRepairing === false) {
                    setTimeout(function () {
                        for (var x in Apio.connectedSockets) {
                            if (x === "admin" || validator.isEmail(x)) {
                                var socketIds = Apio.connectedSockets[x];
                                for (var i in socketIds) {
                                    if (apioId === Apio.io.sockets.connected[socketIds[i]].client.request.session.apioId) {
                                        Apio.io.sockets.connected[socketIds[i]].emit("uncompleted_detected", "recovered");
