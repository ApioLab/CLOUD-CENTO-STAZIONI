...
app.get("/apio/dongle/getSettings", function (req, res) {
    ...
    if (Apio.Serial.serialPort && Object.keys(Apio.Serial.serialPort).length && Apio.Serial.serialPort.isOpen()) {
...
app.get("/apio/dongle/getOpening", function (req, res) {
    if (Apio.Serial.serialPort && Object.keys(Apio.Serial.serialPort).length) {
...
app.post("/apio/dongle/onoff", function (req, res) {
    ...
    if (req.body.onoff) {
        ...
        Apio.Serial.serialPort.close(function (e) {
            if (e) {
                console.log("Error while closing serial port: ", e);
            } else {
                Apio.Serial.serialPort.removeAllListeners();
                clearInterval(Apio.Serial.sendDongleInterval);
                Apio.Serial.serialPort = null;
            }
        });
        res.status(200).send();
    }
...
app.get("/apio/dongle/updateDongle", function (req, res) {
    var uri = Apio.Configuration.serial.updateURL || "https://raw.githubusercontent.com/ApioLab/updates/master/Coordinator.cpp.hex";
    var path = "file.cpp.hex";
                    ...
                    if (error) {
                        ...
                        fs.writeFile(path, body, function (e_w) {
                            if (e_w) {
                                ...
                                exec("sudo node  ../apioHexInstaller.js --serial " + Apio.Configuration.serial.port + " --name " + path, function (error_exec) {
                                    if (error_exec) {
                                        res.sendStatus(500);
                                    } else {
...
Apio.Serial.searchDongle = function () {
    console.log("SONO DENTRO Apio.Serial.searchDongle");
            ...
            if (err) {
                ...
                ports.forEach(function (port) {
                    ...
                    if (String(port.manufacturer) === "Apio Dongle" || String(port.manufacturer) === "Apio_Dongle") {
                        console.log("Chiamo Apio.Serial.init() con porta " + String(port.comName));
...
Apio.Serial.init = function () {
    ...
    //Apio.Configuration.serial.port = String(port.comName);
    console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
    console.log("Creo istanza");
    console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
    Apio.Serial.serialPort = new com(Apio.Configuration.serial.port, {
        //baudrate: "2000000",
        baudrate: Apio.Configuration.serial.baudrate || "1000000",
        ...
        setTimeout(function () {
            ...
            Apio.Serial.serialPort.on("data", function (serialString) {
                ...
                if (serialString === "COORDINATOR ACTIVE" || (CCounter == 0 && serialString == "c")) {
                    ...
                    Apio.Serial.queue = [];
                    Apio.Serial.incoming = [];
                    //send_on_console("Istanzio la coda degli invii")
                    //Apio.Serial.sendInstance=true;
                    Apio.Serial.sendInterval();

                    Apio.Serial.isInIncoming = function (data) {
                        for (var x in Apio.Serial.incoming) {
                            if (data.msg === Apio.Serial.incoming[x].msg) {
                                return x;
                            }
                        }
                    ...
                    Apio.Serial.canSend = function (data) {
                        var msgComponents = data.msg.split(":");
                        var toCheck = msgComponents[0] + ":" + msgComponents[1] + ":" + msgComponents[2];
                        var ts0 = undefined;

                        for (var x in Apio.Serial.incoming) {
                            if (data.msg === Apio.Serial.incoming[x].msg) {
                                ts0 = Apio.Serial.incoming[x].ts;
                            }
                        }

                        for (var x in Apio.Serial.incoming) {
                            if (data.msg !== Apio.Serial.incoming[x].msg && Apio.Serial.incoming[x].msg.indexOf(toCheck) > -1 && ts0 && ts0 < Apio.Serial.incoming[x].msg) {
                                return true;
                            }
                        }
                ...
                if (tokens[0] == "ok") {
                    ...
                    if (Apio.Util.isSet(dataObject.objectId) && Apio.Util.isSet(dataObject.command)) {
                        ...
                        //AGGIORNAMENTO PROSSIMA RELEASE
                        // var ts = new Date().getTime(), index = Apio.Serial.isInIncoming({msg: serialString, ts: ts});
                        // console.log("index: ", index);
                        // if (index > -1) {
                        //     console.log("canSend: ", Apio.Serial.canSend({msg: serialString, ts: ts}));
                        //     console.log("diff: ", ts - Apio.Serial.incoming[index].ts);
                        // }
                        // if (index === -1 || Apio.Serial.canSend({msg: serialString, ts: ts}) || ts - Apio.Serial.incoming[index].ts >= 2000) {
                        //     if (index === -1) {
                        //         Apio.Serial.incoming.push({msg: serialString, ts: ts});
                        //     } else {
                        //         Apio.Serial.incoming[index] = {msg: serialString, ts: ts};
                        //     }
                        //
                        //     console.log("§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§");
                        //     console.log("INVIO");
                        ...
                        //     Apio.Serial.read(dataObject2);
                        //
                        //     if (Apio.Serial.incoming.length >= 10) {
                        //         for (var k = 0; k < Apio.Serial.incoming.length; k++) {
                        //             if (ts - Apio.Serial.incoming[k].ts >= 2000) {
                        //                 Apio.Serial.incoming.splice(k--, 1);
                        //             }
                        //         }
                        //     }
                        // }
        ...
        if (Apio.Serial.cMs - Apio.Serial.exCMs >= 10000) {
            ...
            if (Apio.Serial.available && CCounter == exCCounter) {
                ...
                /*CCounterTry ++;
                 ...
                 //Apio.Serial.searchDongle();*/
                console.log("PRIMA DI PROCESS EXIT");
                process.exit();
            }
            else {
                exCCounter = CCounter;
...
Apio.Serial.sendInterval = function () {
        ...
        if (Apio.Serial.available && Apio.Serial.queue.length) {
            ...
            if (typeof messageToSend !== "undefined" && messageToSend !== null) {
                if (typeof messageToSend === "object") {
                    ...
                    Apio.Serial.verifyQueue.push(JSON.parse(JSON.stringify(messageToSend)));
    ...
    Apio.Serial.verifyQueueVerification = setInterval(function () {
        ...
        //console.log("------------verify-------------")
        //console.log("Apio.Serial.verifyQueue (1): ", Apio.Serial.verifyQueue);
        //var firstMessage = Apio.Serial.verifyQueue[0].message.split(":");
        
        for (var i = 0; i < Apio.Serial.verifyQueue.length; i++) {
            if (Apio.Serial.verifyQueue[i] !== null) {
                //if (Apio.Serial.verifyQueue[i] !== undefined && ts - Apio.Serial.verifyQueue[i].timestamp >= 1500) {
                if (Apio.Serial.verifyQueue[i] !== undefined && ts - Apio.Serial.verifyQueue[i].processTime >= configurationQueue.timerACKWait && ts - Apio.Serial.verifyQueue[i].timestamp < configurationQueue.timerDiscartQueue * 1000) {
                    ...
                    if (insertFlag) Apio.Serial.verifyQueue.splice(i--, 1);
	                    ...
	                    if(Apio.Serial.verifyQueue[i].counter <= 3){
		                    Apio.Serial.verifyQueue[i].counter++;
		                    Apio.Serial.queue.unshift(Apio.Serial.verifyQueue.splice(i--, 1)[0]);
	                    } else {
		                    Apio.Serial.verifyQueue[i].counter++;
		                    Apio.Serial.queue.push(Apio.Serial.verifyQueue.splice(i--, 1)[0]);
	                    }
                    ...
                    //}
                } else if (ts - Apio.Serial.verifyQueue[i].timestamp > configurationQueue.timerDiscartQueue * 1000) {
                    Apio.Serial.verifyQueue[i] = null;
                    console.log("ELIMINO PER ECCESSIVI FALLIMENTI");
                }
            } else {
                Apio.Serial.verifyQueue.splice(i--, 1);
            }
...
http.listen(port, "localhost", function () {
    ...
    fs.appendFile("log_dongle_apio.txt", "Service dongle_apio started at " + (new Date()), function (err_a) {
        if (err_a) {
            console.log("Error while appending data to file: ", err_a);
        } else {
