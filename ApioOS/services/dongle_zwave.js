...
var zwaveSendToDevice = function (data, addInfo) {
    ...
    if (data.message.property == "switch") {
        ...
        if (communication.hasOwnProperty('zwave') && bindToProperty.hasOwnProperty('zwave')) {
            ...
            var type = '';
            if (bindToProperty.hasOwnProperty('zwave') && bindToProperty.zwave.hasOwnProperty(data.address)) {
                type = bindToProperty.zwave[data.address].type
                console.log('OGGETTO TROVATO NEL SISTEMA ACCETTO IL MESSAGGIO');
                console.log();
                console.log("bindToProperty.zwave[nodeid]: ", bindToProperty.zwave[data.address], "s: ", s, "type: ", type, "s.hasOwnProperty(type): ", s.hasOwnProperty(type));
                if (s.hasOwnProperty(type) && s[type].hasOwnProperty('send')) {
                    var fun = data.message.property;
                    console.log('fun ', fun);
                    s = s[type].send[fun]
                    console.log("Chiamo la send")
    ...
    try {
        if (flagSend) {
            ...
            zwave.setChangeVerified(nodeId, classZ, instance, index, true);
        } else {
	        if(send.hasOwnProperty('config') && send.config==true){
		        console.log("Invio questi sono parametri")
		        //zwave.requestAllConfigParams(data.address)
		        zwave.setConfigParam(data.address, send.index, send.value);
	        } else {
			   	zwave.setValue(data.address, send.classZ, send.instance, send.index, send.value);
	            zwave.setChangeVerified(data.address, send.classZ, send.instance, send.index, true);
        ...
        if (communication.hasOwnProperty('zwave') && bindToProperty.hasOwnProperty('zwave')) {
            ...
            if (bindToProperty.hasOwnProperty('zwave') && bindToProperty.zwave.hasOwnProperty(String(nodeid))) {
                ...
                if (s.hasOwnProperty(type) && s[type].hasOwnProperty('recive')) {
                    if (comclass === 37) {
                        var fun = '00';
                        //Aggiorno lo switch
                    } else if (comclass === 49 && value["label"] === "Temperature") {
                        var fun = '03';
                    } else if (comclass === 49 && value["label"] === "Luminance") {
                        var fun = '04';
                    } else if (comclass === 49) {
                        var fun = '01';
                        //Aggiorno la power
                    } else if (comclass === 50 && value["label"] === "Power") {
                        var fun = '01';
                    } else if (comclass === 50 && value["label"] === "Energy") {
                        var fun = '02';
                    }else if (comclass === 50 && value["label"] === "Gas") {
                        var fun = '00';
                    }else if (comclass === 50 && value["label"] === "Water") {
                        var fun = '00';
                    } else if (comclass === 67) {
                        var fun = '00';
                    } else if (comclass === 113 && value["label"] === "Burglar") {
                        var fun = '00';
                    } else if (comclass === 113 && value["label"] === "Access Control") {
                        var fun = '02';
            ...
            //Copiare autoinstallazione node ready
                ...
                if (flagInit) {
                    ...
                    var eepZ = "";
                    if (nodes[nodeid].product.split(' ')[0].length > 1) {
                        eepZ = nodes[nodeid].product.split(' ')[0];
                    } else {
                        eepZ = nodes[nodeid].product
                    }
                    eepZ = eepZ.replace(/ /g, '');
                    console.log("**********EEPZ******** ", JSON.stringify(eepZ))
...
zwave.on("node available", function (nodeid, nodeinfo) {
    ...
    if (flagInit) {
        ...
        //console.log(nodeinfo.product)
        var eepZ = "";
        if (nodeinfo.product.split(' ')[0].length > 1) {
            eepZ = nodeinfo.product.split(' ')[0];
        } else {
            eepZ = nodeinfo.product
        }
        eepZ = eepZ.replace(/ /g, '');
        console.log("**********EEPZ******** ", JSON.stringify(eepZ))
        console.log(JSON.stringify(nodes[nodeid]))
