        ...
        if (!obj.hasOwnProperty(data.objectId)) {
            ...
            for (var i in data.properties) {
                if (obj[data.objectId].properties.hasOwnProperty(i) && obj[data.objectId].properties[i].value !== data.properties[i]) {
                    if (prevObj.hasOwnProperty(data.objectId) && prevObj[data.objectId].properties.hasOwnProperty(i)) {
                        prevObj[data.objectId].properties[i].value = obj[data.objectId].properties[i].value;
                    } else {
                        prevObj[data.objectId] = JSON.parse(JSON.stringify(obj[data.objectId]));
                    }
...
app.post("/apio/logic/modifyFile", function (req, res) {
    ...
    setTimeout(function () {
        try {
            o.loop = require("./apio_logic/" + req.body.newName)(Apio.logic, request, socketServer, Apio.io);
...
app.post("/apio/logic/newFile", function (req, res) {
    clearInterval(loop);
    for (var i in logics) {
        if (logics[i].name === req.body.newName) {
            require.uncache("./apio_logic/" + req.body.newName);
            logics.splice(i, 1);
            break;
        }
    }
...
Apio.logic = {
    ...
    setProperty: function (id, p, v, writeDb, writeSerial) {
	    //console.log("setProperty", obj[id].properties[p]," ",obj[id].properties[p].value);
        if (typeof obj[id] === 'undefined' && !writeDb) {
            ...
            o.properties[p] = v;
            //console.log("apio_client_update 0");
            ...
            o.properties[p] = v;
            //console.log("apio_client_update 1");
            Apio.io.emit("apio_client_update", o);
            ...
            o.writeToSerial = true;
            o.properties[p] = v;
            //console.log("apio_client_update 2");
            Apio.io.emit("apio_client_update", o);
        }
    },
    ...
    apps: function(){
	    this.get = function (data,callback) {
		    if(!this.queryMongoFlag){
			    this.queryMongoFlag = 1;
				this.dbClient.connect("mongodb://" + Apio.Configuration.database.hostname + ":" + Apio.Configuration.database.port + "/" + Apio.Configuration.database.database, function (error, db) {
			        if (error) {
			            // console.log("Unable to get database");
			        } else if (db) {
			            // console.log("Connected to DB");
			            db.collection("Objects").find(data).toArray(function (err, result) {
			                if (err) {
			    ...
			    //console.log(now-this.previousQuery)
			    if(now-this.previousQuery>60000){
				    this.queryMongoFlag = 0;
				    this.previousQuery = new Date().getTime();
			    }
		    }
		}
	    this.queryMongoFlag = 0,
    ...
    logs : function(){
	    this.get = function (objId,property,fromDate,toDate,callback) {
		    if(!this.queryLogsFlag){
			    this.queryLogsFlag = 1;
			    console.log("SELECT "+property+",timestamp FROM `" + objId + "` WHERE (`timestamp` >= " + fromDate + " AND `timestamp` <= " + toDate + ") ORDER BY timestamp DESC")
				this.logsDb.query("SELECT "+property+",timestamp FROM `" + objId + "` WHERE (`timestamp` >= '" + fromDate + "' AND `timestamp` <= '" + toDate + "') ORDER BY timestamp DESC", function (error, result) {
		            if (error) {
			    ...
			    var now = new Date().getTime();
			    console.log(now-this.previousQueryLogs)
			    if(now-this.previousQueryLogs>60000){
				    this.queryLogsFlag = 0;
				    this.previousQueryLogs = new Date().getTime();
			    }
		    }      
	    }
	    this.logsDb = mysql.createConnection("mysql://root:root@127.0.0.1/Logs");
        this.queryLogsFlag = 0;
		this.previousQueryLogs = new Date().getTime();
	}
...
var initialSetup = function () {
    for (var i in files) {
        var o = {};
        try {
            o.loop = require("./apio_logic/" + files[i])(Apio.logic, request, socketServer, Apio.io);
            o.name = files[i];
            logics.push(o);
        } catch (ex) {
...
socketServer.on("connection", function (Socket) {
    ...
    Socket.on("apio_logic_modify", function (data) {
        ...
        for (var i = 0; !found && i < logics.length; i++) {
            if (logics[i].name === data.newName) {
                ...
                try {
                    ...
                    logics[i].loop = require("./apio_logic/" + data.newName)(Apio.logic, request, socketServer, Apio.io);
                } catch (ex) {
                    // socketServer.emit("send_to_client", {
                    //     apioId: Apio.System.getApioIdentifier(),
                    //     message: "logic_error",
        ...
        if (!found) {
            try {
                logics.push({
                    loop: require("./apio_logic/" + data.newName)(Apio.logic, request, socketServer, Apio.io),
                    name: data.newName
                });
            } catch (ex) {
                // socketServer.emit("send_to_client", {
