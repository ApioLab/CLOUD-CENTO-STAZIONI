...
module.exports = function (Apio) {
    return {
        ...
        createNewApioApp: function (req, res) {
            ...
            } else if (!mongoObj.hasOwnProperty("user")) {
                mongoObj.user = [];
            }
                    ...
                    Apio.Database.db.collection("Services").find().toArray(function (err1, services) {
                        if (err1) {
                            ...
                            try {
                                ...
                                //LOGICS - BEGIN
                                var logics = fs.readdirSync("services/apio_logic");
                                for (var x in logics) {
                                    if (logics[x].indexOf(".js") > -1 && id + "_" === logics[x].substr(0, (id + "_").length)) {
                                        if (!fs.existsSync(path + "/" + dummy + "/apio_logic")) {
                                            fs.mkdirSync(path + "/" + dummy + "/apio_logic");
                                        }

                                        var logicNameParts = logics[x].split("_");
                                        var logicBaseName = "";
                                        for (var i = 1; i < logicNameParts.length; i++) {
                                            if (logicBaseName === "") {
                                                logicBaseName = logicNameParts[i];
                                            } else {
                                                logicBaseName += "_" + logicNameParts[i];
                                            }
                                        }

                                        var file = fs.readFileSync("services/apio_logic/" + logics[x], "utf8").replace(new RegExp("\"" + id + "\"", "g"), "\"" + dummy + "\"").replace(new RegExp("'" + id + "'", "g"), "\"" + dummy + "\"");
                                        fs.writeFileSync(path + "/" + dummy + "/apio_logic/" + dummy + "_" + logicBaseName, file);
                                    }
                                }
                                //LOGICS - END
                                ...
                                targz().compress(path + "/" + dummy, path + "/" + object.mongo.name + ".tar.gz", function (err) {
                                    if (err) {
                                        console.log(err);
                                    } else {
        ...
        exportInoApioApp: function (req, res) {
            ...
            try {
                targz().compress(path + "/" + obj.objectId + "/_" + obj.objectId, path + "/" + obj.objectId + "/" + obj.objectId + ".tar.gz", function (err) {
                    if (err) {
                        console.log(err);
                    } else {
        ...
        deleteApioApp: function (req, res) {
            ...
            var deleteThisObjectToDB = function (o) {
                Apio.Database.getObjectById(o, function (result) {
                    ...
                    ///VERSIONE VECCHIA FUNZIONA SOLO CON AUTOINSTALLAZIONI DA LOCALE
                    // if (result.hasOwnProperty("appId")) {
                    //     fs.readdir("public/applications/newfile/" + result.appId + "/apio_logic", function (err_dir, files) {
                    //         if (err_dir) {
                    //             console.log("Error while reading directory: ", err_dir);
                    //         } else if (files) {
                    ...
                    //                 } else if (service) {
                    //                     files.forEach(function (logic) {
                    //                         if (logic.indexOf(".js") > -1) {
                    //                             request.post("http://localhost:" + service.port + "/apio/logic/delete", {
                    //                                 json: true,
                    //                                 body: {
                    //                                     name: result.objectId + "_" + logic
                    //                                 }
                    //                             }, function (err, response) {
                    //                                 if (err) {
                    //                                     console.log("Error while creting new logic: ", err);
                    //                                 } else if (response && Number(response.statusCode) === 200) {
                    //                                     console.log("Logic " + result.objectId + "_" + logic + " successfully created");
                    //                                 } else {
                    ...
                    Apio.Database.db.collection("Objects").findOne({objectId :result.objectId,"logics" : {$exists: true}},{logics:1, _id:0},function(err,files){
                        if (err) {
                            console.log("Error while reading directory: ", err_dir);
                        } else if (files) {
                            ...
                            Apio.Database.db.collection("Services").findOne({name: "logic"}, function (err_service, service) {
                                if (err_service) {
                                    ...
                                    files.logics.forEach(function (logic) {
                                        if (logic.indexOf(".js") > -1) {
                                            request.post("http://localhost:" + service.port + "/apio/logic/delete", {
                                                json: true,
                                                body: {
                                                ...
                                                if (err) {
                                                    console.log("Error while deleting logic: ", err);
                                                } else if (response && Number(response.statusCode) === 200) {
                                                    console.log("Logic " + logic + " successfully deleted");
                                                } else {
                                                    console.log("Either no response or an error occurred");
                                                }
                                            });
        ...
        uploadApioApp: function (req, res) {
            ...
            form.on("file", function (name, file) {
                ...
                targz().extract("upload/" + file.name, "upload/temp", function (err) {
                    if (err)
                        console.log(err);
                    ...
                    Apio.Database.getMaximumObjectId(function (error, data) {
                        if (error) {
                            ...
                            if (!object.mongo.hasOwnProperty("user")) {
                                object.mongo.user = [];
                            }
                            ...
                            Apio.Database.registerObject(object.mongo, function (err) {
                                if (err) {
                                    ...
                                    //LOGICS - BEGIN
                                    if (fs.existsSync("upload/temp/" + id + "/apio_logic")) {
                                        Apio.Database.db.collection("Services").findOne({name: "logic"}, function (err_service, service) {
                                            if (err_service) {
                                                console.log("Error while getting service logic: ", err_service);
                                            } else if (service) {
                                                var request = require("request");
                                                var logics = fs.readdirSync("upload/temp/" + id + "/apio_logic");
                                                logics.forEach(function (logic, index, ref_array) {
                                                    var logicBaseName = logic.replace(/\*_TMP_\*/g, dummy);
                                                    fs.readFile("upload/temp/" + id + "/apio_logic/" + logic, "utf8", function (e, content) {
                                                        if (e) {
                                                            console.log("Error while reading file: ", e);
                                                        } else if (content) {
                                                            content = content.replace(/\*_TMP_\*/g, dummy);
                                                            request.post("http://localhost:" + service.port + "/apio/logic/newFile", {
                                                                json: true,
                                                                body: {
                                                                    newName: logicBaseName,
                                                                    file: content
                                                                }
                                                            }, function (err, response) {
                                                                if (err) {
                                                                    ...
                                                                    console.log("Logic " + dummy + "_" + logicBaseName + " successfully created");
                                                                } else {
                                                                    console.log("Either no response or an error occurred");
                                                                }
