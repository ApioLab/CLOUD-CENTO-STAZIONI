...
process.on("uncaughtException", function (exp) {
    if (exp.code === "MODULE_NOT_FOUND") {
        var exec = require("child_process").exec;
        exec("bash relaunch_npm.sh", function () {
            exec("ps aux | grep 'monitor app.js' | grep -v 'grep' | grep -v 'ps'", function (error, stdout) {
                stdout = stdout.trim().split("\n");
                if (stdout[0]) {
                    process.kill(process.pid, "SIGHUP");
                } else {
                    exec("bash ../start.sh", function () {
                        process.kill(process.pid, "SIGHUP");
                    });
                }
            });
...
app.use(function (req, res, next) {
    if (req.headers.host.indexOf("localhost") > -1 || req.headers.host.indexOf("127.0.0.1") > -1) {
        ...
        if (req.path === "/" || (req.path.indexOf("template") > -1 && (req.path.indexOf("logo.png") > -1 || req.path.indexOf("background.jpg") > -1)) || req.path === "/apio/user/authenticate" || req.path.indexOf("/apio/generateAndSendPDF") > -1 || req.path.indexOf("/apio/sync") > -1 || req.path.indexOf("/apio/enableSync") > -1 || req.path === "/apio/user" || req.path === "/apio/assignToken" || req.path.indexOf("/apio/shutdownBoard") > -1 || req.headers.hasOwnProperty("cookie")) {
            next();
        } else {
            res.redirect("/");
        }
...
d.run(function () {
    ...
    Apio.Database.connect(function () {
        ...
        Apio.Database.db.listCollections({name: "Events"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of Events: ", err);
        ...
        Apio.Database.db.listCollections({name: "Objects"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of Objects: ", err);
            } else if (items.length) {
                console.log("Collection Objects Exists");
        ...
        Apio.Database.db.listCollections({name: "Planimetry"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of Planimetry: ", err);
            } else if (items.length) {
                console.log("Collection Planimetry Exists");
        ...
        Apio.Database.db.listCollections({name: "Communication"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of Communication: ", err);
            } else if (items.length) {
                console.log("Collection Communication Exists");
        ...
        if (Apio.Configuration.type === "gateway") {
            ...
            Apio.Database.db.listCollections({name: "Services"}).toArray(function (err, items) {
                if (err) {
                    console.log("Error while check existence of Services: ", err);
                } else if (items.length) {
                    console.log("Collection Services Exists");
        ...
        Apio.Database.db.listCollections({name: "States"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of States: ", err);
            } else if (items.length) {
                console.log("Collection States Exists");
        ...
        Apio.Database.db.listCollections({name: "Users"}).toArray(function (err, items) {
            if (err) {
                console.log("Error while check existence of Users: ", err);
            } else if (items.length) {
                console.log("Collection Users Exists");
    ...
    app.post("/apio/generateAndSendPDF", routes.cloud.generateAndSendPDF);
    ...
    app.post("/apio/user/setSubrole", routes.users.setSubrole);
    ...
    app.put("/apio/service/:service/route/:route/data/:data", routes.services.putRequest);
    app.get("/apio/service/:service/route/:route", routes.services.getRequest);
    app.delete("/apio/service/:service/route/:route/data/:data", routes.services.deleteRequest);
    ...
    http.listen(Apio.Configuration.http.port, function () {
        ...
        if (Apio.Configuration.type === "cloud") {
            ...
            //
            //         if (user) {
            //             fs.stat(__dirname + "/relaunch_ppp.sh", function (error_3g, stats_3g) {
            //                 if (error_3g) {
            //                     console.log("Error while getting starts of " + __dirname + "/relaunch_ppp.sh: ", error_3g);
            //                 } else if (stats_3g) {
            //                     fs.stat("/var/spool/cron/crontabs/" + user, function (error_s, stats_s) {
            //                         if (error_s) {
            //                             console.log("No crontab existing, creating file");
            //                             fs.writeFile("/var/spool/cron/crontabs/" + user, "*/10 * * * * sudo bash " + __dirname + "/relaunch_ppp.sh\n1 4 * * * sudo pkill node && sudo bash /home/" + user + "/start.sh\n", function (error) {
            //                                 if (error) {
            //                                     console.log("Error while creating new crontab: ", error);
            //                                 } else {
            //                                     console.log("New crontab successfully created");
            ...
            //                                     console.log("Error while reading /var/spool/cron/crontabs/" + user + ": ", error_r);
            //                                 } else if (content) {
            //                                     if (content.indexOf("sudo bash " + __dirname + "/relaunch_ppp.sh") === -1) {
            //                                         fs.appendFile("/var/spool/cron/crontabs/" + user, "*/10 * * * * sudo bash " + __dirname + "/relaunch_ppp.sh\n", function (error_a1) {
            //                                             if (error_a1) {
            //                                                 console.log("Error while appending on /var/spool/cron/crontabs/" + user + ": ", error_a1);
            //                                             } else if (content.indexOf("sudo pkill node && sudo bash /home/" + user + "/start.sh") === -1) {
            //                                                 fs.appendFile("/var/spool/cron/crontabs/" + user, "1 4 * * * sudo pkill node && sudo bash /home/" + user + "/start.sh\n", function (error_a2) {
            //                                                     if (error_a2) {
            ...
            //                     });
            //                 } else {
            //                     console.log("Unable to get starts of " + __dirname + "/relaunch_ppp.sh, probably file is corrupted");
            //                 }
            //             });
            //         } else {
            //             console.log("Unable to find any user, cron will be not installed");
                ...
                fs.stat(__dirname + "/relaunch_ppp.sh", function (error_3g, stats_3g) {
                    if (error_3g) {
                        ...
                        fs.stat("/var/spool/cron/crontabs/" + user, function (error_s, stats_s) {
                            if (error_s) {
                                console.log("No crontab existing, creating file");
                                fs.writeFile("/var/spool/cron/crontabs/" + user, "*/7 * * * * sudo bash " + __dirname + "/relaunch_ppp.sh\n1 4 * * * sudo pkill node && sudo bash /home/" + user + "/start.sh\n", function (error) {
                                    if (error) {
                                        console.log("Error while creating new crontab: ", error);
                                    } else {
                                        console.log("New crontab successfully created");
                                ...
                                fs.readFile("/var/spool/cron/crontabs/" + user, "utf8", function (error_r, content) {
                                    if (error_r) {
                                        ...
                                        for (var i = 0; i < content.length; i++) {
                                            if (content[i].indexOf("relaunch_ppp.sh") > -1) {
                                                var rowComponents = content[i].split(" ");
                                                if (rowComponents[0] !== "*/7") {
                                                    content[i] = "*/7 * * * * sudo bash " + __dirname + "/relaunch_ppp.sh";
                                                }
                                            } else if (content[i].indexOf("sudo pkill node && sudo bash /home/" + user + "/start.sh") > -1) {
                                                var rowComponents = content[i].split(" ");
                                                if (rowComponents[0] !== "1" || rowComponents[1] !== "4") {
                                                    content[i] = "1 4 * * * sudo pkill node && sudo bash /home/" + user + "/start.sh";
                                                }
                                            } else if (content[i].indexOf(__dirname + "/3GConnect.js") > -1 || content[i].indexOf("reboot") > -1) {
                                                content.splice(i--, 1);
                                            }
                                        }

                                        fs.writeFile("/var/spool/cron/crontabs/" + user, content.join("\n") + "\n", function (error) {
                                            if (error) {
                                                console.log("Error while creating new crontab: ", error);
                                            } else {
                                                console.log("New crontab successfully created");
        ...
        fs.readFile("/etc/default/rcS", "utf8", function (err, content) {
            if (err) {
                console.log("Error while reading file /etc/default/rcS:", err);
            } else if (content.indexOf("FSCKFIX=yes") === -1) {
                fs.appendFile("/etc/default/rcS", "FSCKFIX=yes\n", function (err) {
                    if (err) {
                        console.log("Error while appending fs check on file /etc/default/rcS: ", err);
                    } else {
                        console.log("Fs check successfully appended on file /etc/default/rcS");
                    }
                });
            } else {
                content = content.split("\n");
                var found = false;
                for (var i = 0; !found && i < content.length; i++) {
                    if (content[i].indexOf("FSCKFIX=yes") > -1) {
                        found = true;
                        if (content[i][0] === "#") {
                            content[i] = content[i].substr(1);
                            fs.writeFile("/etc/default/rcS", content.join("\n"), function (err) {
                                if (err) {
                                    console.log("Error while writing file /etc/default/rcS: ", err);
                                } else {
                                    console.log("File /etc/default/rcS successfully written");
                                }
                            });
                        }
                    }
        ...
        if (Apio.Configuration.type === "gateway") {
            ...
            var final = function () {
                Apio.System.setRepair(false);
            };

            checkNpm(checkBower, final);

            fs.stat("/etc/mysql/conf.d/new_path.cnf", function (error_mysql) {
                if (error_mysql) {
                    fs.writeFile("/etc/mysql/conf.d/new_path.cnf", "[mysqld]\nsecure_file_priv=\"\"\n", function (err_w) {
                        if (err_w) {
                            console.log("Error while writing file /etc/mysql/conf.d/new_path.cnf: ", err_w);
                        } else {
                            exec("service mysql restart", function (e_err) {
                                if (e_err) {
                                    console.log("Error while restarting MySQL: ", e_err);
                                } else {
                                    console.log("MySQL successfully restarted");
                                }
                            });
                        }
                    });
