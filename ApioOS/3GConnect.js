...
var PINPower3GModule = new Gpio(17, "out");
var PIN14 = new Gpio(14, "in");
    ...
    com.list(function (err, ports) {
        if (err) {
            ...
            if (tty) {
                ...
                var ponTty = undefined;
                for (var p = 0; ponTty === undefined && p < ports.length; p++) {
                    if (ports[p].comName.indexOf("AMA0") > -1) {
                        ponTty = ports[p].comName;
                    }
                }

                if (ponTty && Number(PIN14.readSync()) === 1) {
                    exec("ifconfig ppp0", function (err_ppp, stdout) {
                        stdout = stdout.split("\n");
                        stdout.pop();
                        if (stdout.length !== 8) {
                            setTimeout(function(){
                                PINPower3GModule.writeSync(1);
                                setTimeout(function(){
                                    PINPower3GModule.writeSync(1);
                                    setTimeout(function(){
                                        //exec("poff -a", function () {
                                            spawn("pon", {
                                                detached: true
                                            });
                                        //});
